const { DB_TABLE_TEST_HISTORIES, DB_TABLE_TESTS, DB_TABLE_TEST_QUESTIONS, DB_TABLE_TEST_ANSWER_HISTORIES, DB_TABLE_TEST_PARTS } = require("../config/config");
...
class TestService {
    ...
    getTests = async (userID, testCategoryURL) => {
        let QUERY = "";
        let categoryQuery = "";
        const category = this.testCategories.find((item) => item.part === testCategoryURL);
        if (category) {
    ...
    getTestDetailBySelectedPart = async (userID, testID, partOrder) => {
        if (userID) {
    ...
    getTestHistories = async (userID, testID) => {
        if (userID && testID) {
            const QUERY = `SELECT * FROM ${DB_TABLE_TEST_HISTORIES} h WHERE userID = ? AND testID = ? 
            ...
            return { result: false, msg: "No UserID or No TestID" };
        }
    };

    saveUserTestProgress = async (userID, testData) => {
        ...
        if (userID) {
            const EVALUATE_QUERY = `SELECT id, answer from ${DB_TABLE_TEST_QUESTIONS} WHERE partOrder in (${partArrSQLString})`;
            try {
                ...
                if (result.length > 0) {
                    ...
                    try {
                        ...
                        const QUERY = `INSERT INTO ${DB_TABLE_TEST_HISTORIES} (id, userId, testId, partArr, duration, startingTime, noOfCorrectQuestions, totalQuestions) 
            VALUES ('${historyID}', '${userID}', '${testID}', '${partArr}', ${duration}, '${startingTime}', ${noOfCorrectQuestions}, ${result.length})`;
                        await connection.query(QUERY);

                        for (let i = 0; i < answerArr.length; i++) {
                            const ANSWER_HISTORY_QUERY = `INSERT INTO ${DB_TABLE_TEST_ANSWER_HISTORIES} (historyID, userID, questionID, answer, testId, result) 
    ...
    getTestResult = async (userID, historyID) => {
        if (userID) {
            const HISTORY_QUERY = `SELECT * FROM ${DB_TABLE_TEST_HISTORIES} h 
            INNER JOIN ${DB_TABLE_TESTS} t ON t.id = h.testID 
            WHERE h.id = ? AND h.userID = ?`;
            const QUESTION_QUERY = `SELECT h.answer as userAnswer, h.result, q.answer as answer, q.id as questionID, q.questionContent, q.questionOrder, q.partOrder, q.answerArr, q.qType FROM ${DB_TABLE_TEST_ANSWER_HISTORIES} h
            JOIN ${DB_TABLE_TEST_QUESTIONS} q ON
            q.id = h.questionID
            ...
            try {
                ...
                if (historyResult.length === 1 && questionResult.length > 0) {
                    ...
                    const PART_QUERY = `SELECT partContent, partOrder, audioFile, sectionArr FROM ${DB_TABLE_TEST_PARTS} WHERE testID = ? AND partOrder in (${partArrSQLString}) ORDER BY partOrder ASC`;
                    try {
